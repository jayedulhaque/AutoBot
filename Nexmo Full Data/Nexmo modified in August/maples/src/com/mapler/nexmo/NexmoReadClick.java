...
public class NexmoReadClick implements Runnable {
    ...
    private void _readMessage(AuthInfo authInfo) {
        iNotifier.notify("Going to read message from Nexmo.");
        log.debug("Going to read message from Nexmo.");
        ...
        try {
            ...
            if (StringUtils.isBlank(response) || response.contains("Request failed.")) {
                iNotifier.notify("Nexmo::Can not read message");
                log.debug("Nexmo::Can not read message");
                Util.wait(1);
                return;
            }
